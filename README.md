...
Run `python3 adversarial.py` — 266 checks written to falsify these claims, not
...
| file | what |
|---|---|
| `adversarial.py` | **266 falsification checks + 26 forgery attacks.** Start here. |
...
| `m_star.py` | the `m` hinge is the query budget; `m*(s)` and what the `m≥3` floor costs |
